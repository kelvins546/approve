...
$sql_found = "SELECT id, item_name, category, location_found, date_found, time_found FROM pending_claim_reports WHERE user_id = ?";
...
$sql_found = "SELECT id, item_name, category, location_found, date_found, time_found FROM approved_claim_reports WHERE user_id = ?";
$stmt_found = $conn->prepare($sql_found);
$stmt_found->bind_param("s", $user_id);
$stmt_found->execute();
$result_found = $stmt_found->get_result();
    ...
    if (in_array($table, $valid_tables)) {
        ...
        if ($stmt->execute()) {
            echo "<script>
                    ...
                    window.location.href = 'usersoloviewclaim.php'; // Redirect to the same page
                  </script>";
        } else {
...
function countUserReports($conn, $userId)
{
    $query = "
        SELECT COUNT(*) AS total_reports FROM (
            SELECT id FROM pending_claim_reports WHERE user_id = ?
            UNION ALL
            SELECT id FROM approved_claim_reports WHERE user_id = ?
        ) AS user_reports
    ";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['table'], $_POST['delete_id'])) {
        ...
        $valid_tables = ['pending_claim_reports', 'approved_claim_reports'];
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    echo "<script>
                            ...
                            window.location.href = 'usersoloviewclaim.php';
                          </script>";
                } else {
...
<body>
    ...
    <div class="container-wraping">
        ...
        <div class="container">
            ...
            FROM pending_claim_reports
            ORDER BY date_found DESC";
                $result_pending_found = $conn->query($sql_pending_found);

                if (!$result_pending_found) {
                    die("Error in Pending Claim Reports Query: " . $conn->error);
                }
            ...
            ORDER BY date_found DESC";
                ...
                <div class="report-box2">
                    ...
                    <!-- Report Buttons -->
                    <div class="report-buttons">
                        <a href="usersoloapprovedclaimdetails.php?report_id=<?= $row['id'] ?>" class="btn-action">View
                            Item</a>
                        <form method="get" action="usersoloapprovedclaimdetails.php">
                            <input type="hidden" name="action" value="edit">
                            <input type="hidden" name="id" value="<?= htmlspecialchars($row['id']); ?>">
                            <input type="hidden" name="table" value="approved_claim_reports">
